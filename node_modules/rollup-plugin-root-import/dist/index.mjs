 ...
 * input is an array, filter out all the non-string elements. If we end up with an empty array,
 * return null.
 *
 * @param {*} array
 ...
 * that will reject with the first error evaluate produces, or resolve to the first truthy value which
...
function resolve(importee, imports) {
  return firstOf(imports, async ({
    root: rootPath,
    extension
  }) => {
    const file = path.join(rootPath, importee + extension);
    return fsStat(file).then(stats => stats.isFile() ? file : null, () => null);
  });
}
...
function rootImport(options) {
  ...
  if (!extensions) extensions = [''];else if (!extensions.includes('')) extensions.unshift(''); // We can cache resolved modules when they're relative to the root. We use a null-inherited to
  ...
  return {
    options({
      ...
      if (!roots) roots = []; // Ignore exotic input types (arrays, objects) as may be handled by plugins like
    ...
    resolveId(importee, importer) {
      ...
      if (importee[0] === '/') {
        ...
        const resolution = resolve(importee, imports); // Save this in the cache in case we see it again.
